...
class gmm1:
	...
	#print('RAN THE CLASSIFY SECTION******')
...
#            ll = gmm.eval(features)[0]
	...
	if verbose:
	  ...
	  #print self.count
        ...
        while(True):
            try:
                chunk = sf.read_frames(framelen/2, dtype=np.float32)#read each window sized value from the audio
                sf.seek(-framelen/4,1)#take the current pointer backward for overlap
                if len(chunk) != framelen/2:
                    print("Not read sufficient samples - returning")
                    break
                if sf.channels != 1:
                    chunk = np.mean(chunk, 1) # mixdown
                framespectrum = np.fft.fft(window*chunk,framelen/2) # first the window type is implemented then the padding is done here
...
def trainAndTest(trainpath, trainwavs, testpath, testwavs):
    ...
    #print("TRAINING model load$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$4")
    #model = gmm1(trainpath, trainwavs)
    #model=gmmfl_final_csv.model
    with open('objs.pickle') as f:
        model= pickle.load(f)	
    print(model)
    print(type(model))	
    #print('used pickle success...........................,............................')
    
    #print("TESTING")
    ncorrect = 0
    for wavpath,label in testwavs.items():
	#print('i am here checkk checkk............')
